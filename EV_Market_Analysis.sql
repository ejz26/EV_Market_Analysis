...
-- Q9. Which brands have the highest average horsepower?
SELECT
    brand,
    ROUND(AVG(horsepower)::numeric, 2) AS average_horsepower
FROM ev
GROUP BY brand
ORDER BY average_horsepower DESC
LIMIT 10;

-- Q10. Which vehicles generate the highest estimated revenue?
SELECT
    brand,
    model,
    price_usd,
    annual_sales_units,
    (price_usd * annual_sales_units) AS estimated_revenue
FROM ev
ORDER BY estimated_revenue DESC
LIMIT 10;

-- Q11. Which brands offer the best overall value (range, price, and rating)?
SELECT
    brand,
    ROUND(AVG(range_miles / price_usd)::numeric, 4) AS avg_range_per_dollar,
    ROUND(AVG(customer_rating)::numeric, 2) AS avg_rating,
    ROUND(AVG((range_miles * customer_rating) / price_usd)::numeric, 4) AS overall_value_score
FROM ev
GROUP BY brand
ORDER BY overall_value_score DESC
LIMIT 10;

-- Q12. Which price range has the highest sales?
SELECT
    CASE
...
-- Q13. Which body type has the highest average range?
SELECT
    body_type,
    ROUND(AVG(range_miles)::numeric, 2) AS average_range
FROM ev
...
-- Q14. Which brands dominate each market segment?
SELECT
    market_segment,
    brand,
    SUM(annual_sales_units) AS total_sales
...
-- Q15. Which market segment represents the biggest opportunity for Tesla?
SELECT
    market_segment,
    SUM(annual_sales_units) AS total_sales,
    ROUND(AVG(customer_rating)::numeric, 2) AS avg_rating,
...
-- Q16. Which body type represents the biggest opportunity for Tesla?
SELECT
    body_type,
    SUM(annual_sales_units) AS total_sales,
...
-- Q17. What price range represents the biggest opportunity for Tesla?
SELECT
    CASE
        WHEN price_usd < 30000 THEN 'Under $30k'
        WHEN price_usd BETWEEN 30000 AND 49999 THEN '$30k-$49k'
